...
\def\filedate{April 17, 2017}
\def\fileversion{version 1.2}
...
\item OpenOffice.org and older versions of LibreOffice had no mechanism to 
turn off and on advanced OpenType features, so if you needed to use optional typographic
features in these applications, you had to use SIL Graphite, which is only available
in the TTF version. This limitation has been fixed as of LibreOffice 5.3, which now
has \href{https://wiki.documentfoundation.org/ReleaseNotes/5.3}{full OpenType support}.
...
in the Private Use Area or characters outside of the Unicode 7.0 range. This limitation
has been resolved in LibreOffice (beginning with version 5.3). In earlier versions
of LibreOffice, you needed to rely on \hyperlink{Graphite}{SIL Graphite}
(only available in the TTF versions) for positioning of combining Glagolitic characters.
...
available from the \href{http://sci.ponomar.net/keyboard.html}
...
Under GNU/Linux, this is handled by the |m17n| library and database. See the \href{http://sci.ponomar.net/keyboard.html}
...
and support for turning off and on optional features was added in version 5.3.
...
\hyperlink{Graphite}{Support of Advanced Features in LibreOffice}, below.
...
\hyperlink{Graphite}{Support of Advanced Features in LibreOffice}, below.
Microsoft Office currently has no way to access Stylistic Alternatives and Stylistic Sets.
If necessary, you may access these glyphs by codepoint from the Private
Use Area. However, relying on the PUA as a data exchange mechanism is discouraged.
...
supported in LibreOffice (on GNU/Linux and Windows, starting with
OpenOffice.org version 3.2; on OS X, starting with LibreOffice version 5.3),
Mozilla Firefox (starting with version 11), and \XeTeX{} (starting with version 0.997).
Graphite support is not available in Microsoft Office.

Note that it is currently not possible to add Graphite tables to OpenType-CFF fonts.
Therefore Graphite is only supported in the TrueType versions of fonts.
...
In addition, LibreOffice (as well as other OpenOffice.org derivatives)
automatically recognizes fonts which contain Graphite tables.
For such fonts, Graphite rendering is enabled by default and the
OpenType layout tables will be ignored. OpenType and SIL Graphite
cannot be used simultaneously in LibreOffice, so if you do not want
SIL Graphite layout, you should only use the OpenType-CFF versions of the fonts.

While correct positioning, attachment and substitutions will work automatically both
in the OpenType and SIL Graphite fonts, earlier versions of LibreOffice
had no mechanism to turn off and on optional OpenType features. This
functionality has become available starting with LibreOffice version 5.3.
However, there is still no graphical interface that can be used
to turn such optional features on and off. Instead, a special extended font name
...
the |mark| SIL Graphite feature and display
...
Beginning with LibreOffice version 5.3, the same syntax may be used also
to turn off and on optional OpenType features, such as Stylistic Alternatives (|salt|)
and Stylistic Sets (|ss01|, ...) in the OpenType-CFF versions of the fonts.
For example, to turn on Stylistic Set 1 (|ss01|) in the
font Ponomar Unicode, change the font name to the following:

\begin{verbatim}
Ponomar Unicode:ss01=1
\end{verbatim}

\noindent This feature is not available in Apache OpenOffice;
...
character via Stylistic Set 1 in OpenType and via the |hyph| feature in SIL Graphite.
...
as the OpenType features. The ``Symbol for Mark's Chapter'' (``mark'') feature
provides alternatives for the U+1F545 Symbol for Marks Chapter,
much like the salt feature in OpenType. 
...
\noindent The following additional Graphite features are provided 
(they duplicate the functionality of OpenType features):
...
provides the same functionality as Stylistic Set 1. It
replaces all instances of U+002D Hyphen-Minus and U+2010 Hyphen
with U+005F Low Line (underscore) for use as a hyphenation
character. Possible values are |0| (no) and |1| (yes). Please note that this
...
\newfontfamily{\right}[StylisticSet=1,HyphenChar="200B]{Fedorovsk Unicode}
...
replaces all instances of U+002D Hyphen-Minus and U+2010 Hyphen
with U+005F Low Line (underscore) for use as a hyphenation
character. The Graphite analog is provided by the feature 
``Use underscore for hyphenation'' feature (\emph{hyph}),
which takes values |0| (no) and |1| (yes). Please note that this
feature will be deprecated once the necessary functionality is 
added to LibreOffice.
