...
to truncated letterforms; the order of the alternate forms is always: lower truncation,
...
\begin{itemize}
\item The ``Truncation'' feature (``trnc'') provides the same funcionality
...
replaces all instances of U+002D Hyphen-Minus and U+2010 Hypehn
...
It has been reencoded for Unicode, with added OpenType and Graphite features
...
with values ``left'' and ``right'', which mimicks the behavior
of stylistic sets 1 and 2; and ``Equal Baseline'' (with value ``yes''),
which mimicks the behavior of Stylistic Set 10.
...
reencoded for Unicode by Aleksandr Andreev with permission of the original author.
...
It was edited and reencoded for Unicode by Aleksandr Andreev.
...
{\fontsize{48pt}{2em} \simple\salt СѶНѠ́ДИКЪ.} \\
\end{tabular}
\end{center}
...
\item Ponomar Uniocode has Graphite-based kerning for Cyrillic 
...
please check that your software propertly supports OpenType and / or
SIL Graphite. We suggest checking for expected behaviour in \XeTeX{}
or \LuaTeX{}.
